...
function newsletter_bulkstart_dist($corps,$listes = array(),$options=array()){
	...
	if ($id_mailshot){
		// un updateq a l'identique, qui ne sert a rien sauf si le sujet contient un emoji par exemple :
		// sql_updateq le traite bien mais pas sql_insertq cf _mysql_traite_query
		sql_updateq("spip_mailshots", $bulk, 'id_mailshot='.intval($id_mailshot));
