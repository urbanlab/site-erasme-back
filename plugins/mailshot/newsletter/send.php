...
function newsletter_send_dist($destinataire,$corps,$options=array()){
	...
	if (!$mailer_factory = charger_fonction($config['mailer'],'bulkmailer',true)
	  OR !$mailer = $mailer_factory(
		...
		$url_config = generer_url_ecrire("configurer_mailshot");
		spip_log("mailer non configure pour l'envoi de $sujet a ".$destinataire['email'],'mailshot_send'._LOG_ERREUR);
		return _T('mailshot:erreur_aucun_service_configure',array('url'=>$url_config));
	}
	...
	$sent_via = get_class($mailer) . "[Facteur + " . $config['mailer'] . "]";
	...
	spip_log("mail "."a :".print_r($destinataire['email'],true)."\n"
		. ($head ? trim($head)."\n" : '')
		. "via $sent_via | HTML Length : ".strlen($corps_cont['html']) . " | Texte Length : " . strlen($corps_cont['texte']),'mailshot_send'._LOG_DEBUG);
	...
	if (!$retour) {
		spip_log("Erreur Envoi mail ($dest_email) via $sent_via : ".print_r($mailer->ErrorInfo,true),'mailshot_send'._LOG_ERREUR);
		if ($mailer->ErrorInfo) {
