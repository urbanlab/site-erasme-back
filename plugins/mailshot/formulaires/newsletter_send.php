...
function formulaires_newsletter_send_charger_dist($id_newsletter,$mode_test=false){
	...
	if (mailshot_fail_ratio_alert_check()) {
		return recuperer_fond("prive/squelettes/contenu/inc-mailshot-alerte-fail-ratio",array());
	}
	...
	if (!$mode_test){
		...
		// extraire_multi
		foreach ($listes_dispos as $k => $l) {
			if (strpos($l['titre'], '<') !== false) {
				$listes_dispos[$k]['titre'] = extraire_multi($l['titre']);
			}
		}

		// et trier
		$listes_triees = [];
		while(count($listes_dispos)) {
			$segments = [];
			$liste = reset($listes_dispos);
			if (strpos($liste['id'], '+') !== false) {
				do {
					$segments[] = array_shift($listes_dispos);
					$liste = reset($listes_dispos);
				} while ($liste and strpos($liste['id'], '+') !== false);

				// on trie le groupe de segments d'une meme liste
				usort($segments, function($a, $b) {
					return strtolower($a['titre']) > strtolower($b['titre']) ? 1 : -1;
				});
				while (count($segments)) {
					$listes_triees[] = array_shift($segments);
				}
			}
			else {
				$listes_triees[] = array_shift($listes_dispos);
			}
		}

		$valeurs['_listes_dispo'] = $listes_triees;
...
function formulaires_newsletter_send_traiter_dist($id_newsletter,$mode_test=false){
	...
	if (mailshot_fail_ratio_alert_check()) {
		// on fail, et le charger va afficher le panneau d'avertissement sur les envois
		return array('message_erreur'=>'echec');
	}
