...
/**
 * Génie des envois en lot
 * @param int $t 	timestamp
 * @return int
 */
function genie_mailshot_bulksend_dist($t){
	spip_log("bulksend:meta_processing:".(empty($GLOBALS['meta']['mailshot_processing']) ? '(empty)' : $GLOBALS['meta']['mailshot_processing']),"mailshot");
	...
	if ($id_mailshot = sql_getfetsel('id_mailshot','spip_mailshots','statut='.sql_quote('init').' AND date_start<'.sql_quote(date('Y-m-d H:i:s')),'','','0,1')){
		...
		mailshot_update_meta_processing(true);
	...
	if (sql_countsel("spip_mailshots","statut=".sql_quote('processing'))){
		...
		if (empty($GLOBALS['meta']['mailshot_processing'])) {
			mailshot_update_meta_processing(true);
		}

		// on ne fait rien si on est en alerte fail, mais on se relance en attendant
		...
		$relance = true;
		if (!$boost){
			$periode = 0; // peut-être utilisé pour relancer la tache si pas finie
				  ...
				  and sql_countsel('spip_mailshots_destinataires','id_mailshot='.intval($next['id_mailshot'])) >= $total) {
					...
					if ($nb_process > 0) {
						while ($nb_process) {
							...
							$url = mailshot_url_boost_send($arg);
...
/**
 * Appeler une url sans attendre le résultat
 * Quasi-copie de la fonction queue_lancer_url_http_async() de ecrire/inc/queue.php
 * qui n'existe qu'à partir de SPIP 3.3
 * @param string $url
 * @return void
 */
