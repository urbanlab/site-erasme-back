...
function linkcheck_objet_verifier(string $objet, int $id_objet, $checklink_async = true) {
	...
	if (
		...
		if (!empty($id_linkchecks)) {
			spip_log(
				"linkcheck_objet_verifier: $objet #$id_objet pas a traiter, on supprime ses vieux liens recensés",
	...
	spip_log(
		"linkcheck_objet_verifier: $objet #$id_objet analysé " . count($liens) . ' liens trouvés',
		'linkcheck' . _LOG_DEBUG
	);
	//on les insere dans la base
	$id_linkchecks = linkcheck_objet_ajouter_liens($objet, $id_objet, $liens);
	spip_log(
		"linkcheck_objet_verifier: $objet #$id_objet analysé " . count($id_linkchecks) . ' liens en base',
		'linkcheck' . _LOG_DEBUG
	);

	if ($checklink_async and !empty($id_linkchecks)) {
		spip_log(
			"linkcheck_objet_verifier: $objet #$id_objet analysé, on lance linkcheck_objet_tester_liens en async",
			'linkcheck' . _LOG_DEBUG
		);
		job_queue_add(
			...
			if ($publie != $sel['publie']) {
				...
				if (!$statut_linkckeck or $publie != $statut_linkckeck) {
					if (!$statut_linkckeck or $statut_linkckeck == '' or $publie == 'oui') {
						sql_updateq('spip_linkchecks', ['publie' => $publie], 'id_linkcheck=' . intval($id_linkcheck));
					} elseif (!sql_getfetsel(
						'publie',
						'spip_linkchecks_liens',
						'id_linkcheck=' . $id_linkcheck . '  AND publie="oui"'
					)) {
						sql_updateq('spip_linkchecks', ['publie' => $publie], 'id_linkcheck=' . intval($id_linkcheck));
					}
				}
...
function linkcheck_objet_ajouter_liens($objet, $id_objet, $liens, $publie = null) {
	...
	spip_log(
		"linkcheck_objet_ajouter_liens: $objet #$id_objet ajout/modif en base des linkchecks #" . implode(
			', #',
			$id_linkchecks
		),
		'linkcheck' . _LOG_DEBUG
	...
	$anciens_liens = sql_allfetsel(
		'id_linkcheck',
		'spip_linkchecks_liens',
		['id_objet=' . intval($id_objet),
		'objet=' . sql_quote($objet),
		sql_in('id_linkcheck', $id_linkchecks, 'NOT'),
	]);
	if (!empty($anciens_liens)) {
		...
		sql_delete(
			...
			['id_objet=' . intval($id_objet),
			'objet=' . sql_quote($objet),
			sql_in('id_linkcheck', $anciens_liens),
		]);
		spip_log(
			"linkcheck_objet_ajouter_liens: $objet #$id_objet suppression des liens vers les anciennes URLs #" . implode(
				', #',
				$anciens_liens
			),
			'linkcheck' . _LOG_INFO_IMPORTANTE
		);
	}
