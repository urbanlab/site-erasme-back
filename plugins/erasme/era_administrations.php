...
function era_upgrade($nom_meta_base_version, $version_cible) {
	...
	$maj['1.0.1'] = [['era_ajouter_mots_cles']];
	$maj['1.0.2'] = [['era_ajouter_mots_cles_champs']];
	cextras_api_upgrade(era_declarer_champs_extras(), $maj['1.0.3']);
	$maj['1.0.4'] = [['era_update_couleurs_onglets']];
	$maj['1.0.5'] = [['era_ajouter_mots_cles_champs']];
...
function era_ajouter_mots_cles() {
	...
	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='Grand Lyon' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'Grand Lyon', 'id_groupe' => $id_groupe, 'type' => 'Entreprises']);
	}

	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='Theoriz' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'Theoriz', 'id_groupe' => $id_groupe, 'type' => 'Entreprises']);
	}
	...
	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='Collaboration' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'Collaboration', 'id_groupe' => $id_groupe, 'type' => 'Méthodologies']);
	}

	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='Compétition' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'Compétition', 'id_groupe' => $id_groupe, 'type' => 'Méthodologies']);
	}

	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='Médiation' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'Médiation', 'id_groupe' => $id_groupe, 'type' => 'Méthodologies']);
	}
	...
	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='IFE' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'IFE', 'id_groupe' => $id_groupe, 'type' => 'Partenaires']);
	}

	if (!$id_mot = sql_getfetsel(
		'id_mot',
		'spip_mots',
		"titre='Planétarium de Vaulx-en-Velin' AND id_groupe=$id_groupe"
	)) {
		$id = sql_insertq(
			'spip_mots',
			['titre' => 'Planétarium de Vaulx-en-Velin', 'id_groupe' => $id_groupe, 'type' => 'Partenaires']
		);
	}
	...
	if (!$id_mot = sql_getfetsel(
		'id_mot',
		'spip_mots',
		"titre='Gestionnaire de contenu sémantique' AND id_groupe=$id_groupe"
	)) {
		$id = sql_insertq(
			'spip_mots',
			['titre' => 'Gestionnaire de contenu sémantique', 'id_groupe' => $id_groupe, 'type' => 'Technologies']
		);
	}

	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='Multitouch' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'Multitouch', 'id_groupe' => $id_groupe, 'type' => 'Technologies']);
	}

	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='RFID' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'RFID', 'id_groupe' => $id_groupe, 'type' => 'Technologies']);
	}
	...
	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='Objet numérique' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'Objet numérique', 'id_groupe' => $id_groupe, 'type' => 'Usages']);
	}

	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='Production inversée' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'Production inversée', 'id_groupe' => $id_groupe, 'type' => 'Usages']);
	}

	if (!$id_mot = sql_getfetsel(
		'id_mot',
		'spip_mots',
		'titre="Projet d\'établissement"' . " AND id_groupe=$id_groupe"
	)) {
		$id = sql_insertq(
			'spip_mots',
			['titre' => 'Projet d\'établissement', 'id_groupe' => $id_groupe, 'type' => 'Usages']
		);
	}
	...
	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='Collège François Truffaut' AND id_groupe=$id_groupe")) {
		$id = sql_insertq(
			'spip_mots',
			['titre' => 'Collège François Truffaut', 'id_groupe' => $id_groupe, 'type' => 'Utilisateurs']
		);
	}
	...
	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='Collège Jean Castagne' AND id_groupe=$id_groupe")) {
		$id = sql_insertq(
			'spip_mots',
			['titre' => 'Collège Jean Castagne', 'id_groupe' => $id_groupe, 'type' => 'Utilisateurs']
		);
	}
	...
	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='Collège Philippe Delorme' AND id_groupe=$id_groupe")) {
		$id = sql_insertq(
			'spip_mots',
			['titre' => 'Collège Philippe Delorme', 'id_groupe' => $id_groupe, 'type' => 'Utilisateurs']
		);
	}
}
...
function era_ajouter_mots_cles_champs() {
	...
	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='musees' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'musees', 'id_groupe' => $id_groupe, 'type' => 'Champs']);
	}

	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='education' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'education', 'id_groupe' => $id_groupe, 'type' => 'Champs']);
	}

	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='solidarite' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'solidarite', 'id_groupe' => $id_groupe, 'type' => 'Champs']);
	}

	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='erasme' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'erasme', 'id_groupe' => $id_groupe, 'type' => 'Champs']);
	}

	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='veille' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'veille', 'id_groupe' => $id_groupe, 'type' => 'Champs']);
	}

	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='projets' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'projets', 'id_groupe' => $id_groupe, 'type' => 'Champs']);
	}

	if (!$id_mot = sql_getfetsel('id_mot', 'spip_mots', "titre='lab' AND id_groupe=$id_groupe")) {
		$id = sql_insertq('spip_mots', ['titre' => 'lab', 'id_groupe' => $id_groupe, 'type' => 'Champs']);
	}
}
...
function era_update_couleurs_onglets() {
	...
	if (sql_getfetsel('couleur_tab', 'spip_rubriques', "titre='Education'") == '') {
		sql_updateq('spip_rubriques', ['couleur_tab' => '#A6D011'], 'titre="Education"');
	}

	if (sql_getfetsel('couleur_tab', 'spip_rubriques', "titre='Musées'") == '') {
		sql_updateq('spip_rubriques', ['couleur_tab' => '#48C1D9'], 'titre="Musées"');
	}

	if (sql_getfetsel('couleur_tab', 'spip_rubriques', "titre='Erasme'") == '') {
		sql_updateq('spip_rubriques', ['couleur_tab' => '#8269E1'], 'titre="Erasme"');
	}

	if (sql_getfetsel('couleur_tab', 'spip_rubriques', "titre='Seniors'") == '') {
		sql_updateq('spip_rubriques', ['couleur_tab' => '#FFAA09'], 'titre="Seniors"');
	}

	if (sql_getfetsel('couleur_tab', 'spip_rubriques', "titre='Technologies'") == '') {
		sql_updateq('spip_rubriques', ['couleur_tab' => '#FF3274'], 'titre="Technologies"');
	}
}
