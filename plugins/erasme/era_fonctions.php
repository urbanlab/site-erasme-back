		...
		if (count($infos) == 2) {
			...
			$texteDecoupe .= '
						<li class="event' . $classeActive . '">
							...
							<p class="date">' . str_replace(['>', '&gt;'], '', $infos[0]) . '</p>
						</li>';
		}
	}
	$texteDecoupe .= '
...
function getInformationsChiffresClef($texteBrut = '') {
	...
	foreach ($lignes as $ligne) {
		...
		$texte = $contenus[0] == '>' ? implode(' ', array_slice($contenus, 2, count($contenus))) : implode(
			' ',
			array_slice($contenus, 1, count($contenus))
		);
 ...
 * @return string
 */
function injecterBlocDescriptifTechnique(
...
function injecterBlocInformations(
	$information_site_web = '',
	$information_mailing = '',
	$information_communaute_utilisateur = '',
