	...
	foreach ($ids as $id_mailsubscribinglist) {
		do {
			...
			foreach ($subs as $sub) {
				...
				sql_updateq('spip_mailsubscriptions', ['actualise_segments' => 0], 'id_mailsubscriber=' . intval($sub['id_mailsubscriber']) . ' AND ' . 'id_mailsubscribinglist=' . intval($sub['id_mailsubscribinglist']));
...
function mailsubscribers_remove_from_meta_update($remove = 'all') {
	// ici on efface ce mailsubscribinglist dans l'update, en reprenant depuis les metas
	lire_metas();
	$update_segments = isset($GLOBALS['meta']['mailsubscriptions_update_segments'])
		? unserialize($GLOBALS['meta']['mailsubscriptions_update_segments'])
		: [];
	...
	if (!$update_segments or $remove === 'all') {
		...
		sql_updateq('spip_mailsubscriptions', ['actualise_segments' => 0]);
		return true;
	}
