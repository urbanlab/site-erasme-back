...
function mailsubscribers_liste_statut_auteur_possibles() {
	// utiliser la globale de declaration si elle est definie
	if (!empty($GLOBALS['liste_des_statuts'])) {
		$possibles = $GLOBALS['liste_des_statuts'];
		$possibles = array_flip($possibles);
...
/**
 * URL reconsent
 * pour reconsentir sur une ou plusieurs listes precises, fournir le ou les ID dans $id_mailsubscribinglist
 * @param string $email
 * @param string $jeton
 * @param int|array $id_mailsubscribinglists
 *   une ou plusieurs listes a confirmer
...
function mailsubscriber_afficher_filtre_segment($segment) {
	...
	return "<div class='segment-filters'>" . implode('<br />', $out) . '</div>';
}
...
function mailsubscribers_is_updating_segment($id_mailsubscribinglist, $id_segment) {
	...
	if (
		isset($update_segments[$id_mailsubscribinglist])
 ...
 * @return array|int
 */
function filtre_mailsubscribers_listes_dist($options = []) {
	include_spip('inc/mailsubscribers');
...
function mailsubscribers_titre_segment($identifiant, $id_segment) {
	...
	if (is_null($listes)) {
		include_spip('inc/mailsubscribers');
		$listes = mailsubscribers_listes(['segments' => true]);
	}
