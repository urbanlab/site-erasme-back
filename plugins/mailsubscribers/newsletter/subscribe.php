 ...
 *   comment : string commentaire sur la raison de l'inscription
 * @return bool
 *   true si inscrit comme demande, false sinon
 */
function newsletter_subscribe_dist($email, $options = []) {
	static $dejala = false;
	...
	$listes = false;
	$trace_optin = '';
	foreach (['lang', 'nom', 'invite_email_from', 'invite_email_text'] as $k) {
		if (isset($options[$k])) {
			$set[$k] = $options[$k];
		}
	}
	...
	if (!is_array($listes)) {
		$listes = [mailsubscribers_normaliser_nom_liste()];
	}
	...
	if (
		...
		foreach ($listes as $identifiant) {
			if (
				...
				if (
					$sub['statut'] == 'prop'
					and (!isset($set['statut']) or !in_array($set['statut'], ['prepa', 'valide']))
					and !in_array($row['statut'], ['prepa', 'valide'])
				) {
					$set['statut'] = 'prop';
		...
		foreach ($notify as $k => $option) {
			if (isset($options['invite_email_from']) and strlen($options['invite_email_from'])) {
				...
				if (isset($options['invite_email_text'])) {
					$notify[$k]['invite_email_text'] = $options['invite_email_text'];
				}
			}
		}
