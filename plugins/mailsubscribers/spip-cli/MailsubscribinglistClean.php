...
class MailsubscribinglistClean extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		$email_alive = sql_allfetsel('distinct email', 'spip_mailshots_destinataires', sql_in('statut', ['clic', 'read']) . ' AND date>' . sql_quote($from));
		...
		$email_vrai_zombies = sql_allfetsel('distinct email', 'spip_mailshots_destinataires', sql_in('statut', ['todo', 'fail'], 'NOT') . ' AND ' . sql_in('email', $email_zombies) . ' AND date>' . sql_quote($from));
		...
		if (
			...
			if (count($id_mailsubscribers_prop)) {
				...
				if (
					...
					sql_updateq('spip_mailsubscribers', ['statut' => 'prop'], sql_in('id_mailsubscriber', $id_mailsubscribers_prop));
		...
		$alive_unsub = sql_allfetsel('email, max(date) as date_max, statut', 'spip_mailshots_destinataires', sql_in('statut', ['clic', 'read']) . ' AND ' . sql_in('email', $emails_unsub) . ' AND date>' . sql_quote($from), 'email');
