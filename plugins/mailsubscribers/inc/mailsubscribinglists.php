...
function mailsubscribers_start_update_mailsubscribinglist_segment($id_mailsubscribinglist, $id_segment) {
	...
	if (!isset($update[$id_mailsubscribinglist])) { $update[$id_mailsubscribinglist] = [];
	}
	...
	$n = 0;
	while (
		$in_subscribers_valides = sql_allfetsel(
			'DISTINCT id_mailsubscriber',
			'spip_mailsubscriptions',
			'statut!=' . sql_quote('refuse') . ' AND actualise_segments=0 AND id_segment=0 AND id_mailsubscribinglist=' . intval($id_mailsubscribinglist),
			'',
			'',
			'0,10000'
		)
		...
		sql_updateq('spip_mailsubscriptions', ['actualise_segments' => 1], 'id_segment=0 AND id_mailsubscribinglist=' . intval($id_mailsubscribinglist) . ' AND ' . sql_in('id_mailsubscriber', $in_subscribers_valides));
