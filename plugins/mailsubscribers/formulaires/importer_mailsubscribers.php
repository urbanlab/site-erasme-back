...
function formulaires_importer_mailsubscribers_verifier_dist() {
	...
	if (!$filename) {
		...
		$erreurs['test'] .= "<p class='explication'>{{" . singulier_ou_pluriel(
			$count,
			'mailsubscriber:info_1_adresse_a_importer',
			'mailsubscriber:info_nb_adresses_a_importer'
		) . '}}</p>';
	...
	$message =
		sinon(
			singulier_ou_pluriel(
				$r['count'],
				'mailsubscriber:info_1_mailsubscriber',
...
function importer_mailsubscribers_data($filename) {
	...
	while ($data_raw and count($data_raw)) {
		...
		foreach (['nom', 'lang', 'listes', 'statut', 'date'] as $k) {
			if (isset($row[$k])) {
				$d[$k] = $row[$k];
			}
		}
 ...
 * @return array
 */
function importer_mailsubscribers_importe($filename, $options = []) {
	$res = ['count' => 0, 'erreurs' => []];
		...
		if ($email and email_valide($email)) {
			...
			if (!isset($d['statut']) or !isset($d['listes'])) {
				if (!mailsubscribers_test_email_obfusque($email)) {
					...
					spip_log("$i/$nb_total. Importer $email " . json_encode($set), 'mailsubscribers');
					$res['count']++;
				}
				...
				if ($id) {
					...
					if (in_array($d['statut'], ['prop','valide','refuse'])) {
						$set = ['listes' => $listes,'notify' => false];
						if ($d['statut'] == 'valide') {
							$set['force'] = true;
						}
						else {
