...
function formulaires_editer_mailsubscriber_identifier_dist(
	...
	return serialize([intval($id_mailsubscriber)]);
}
...
function formulaires_editer_mailsubscriber_verifier_dist(
	...
	$erreurs = formulaires_editer_objet_verifier('mailsubscriber', $id_mailsubscriber, ['email']);
		...
		if (!email_valide($email)) {
			...
			if (
				sql_countsel(
					...
					'email=' . sql_quote($email) . ' AND id_mailsubscriber!=' . intval($id_mailsubscriber)
				) > 0
			) {
				...
				if ($id = sql_getfetsel('id_mailsubscriber', 'spip_mailsubscribers', 'email=' . sql_quote($email), '', '', '0,1')) {
					...
					$erreurs['email'] = _T('mailsubscriber:erreur_adresse_existante_editer', ['url' => $url_page_edit]);
				} else {
...
function formulaires_editer_mailsubscriber_traiter_dist(
	...
	if (
		!intval($id_mailsubscriber)
		and $id = sql_getfetsel(
			'id_mailsubscriber',
			'spip_mailsubscribers',
