...
function formulaires_newsletter_subscribe_traiter_dist($listes = '', $option = '') {
	...
	if (lire_config('mailsubscribers/double_optin', 0)) {
		$res['message_ok'] = _T('newsletter:subscribe_message_ok_confirm', ['email' => "<b>$email</b>"]);
	} else {
		$res['message_ok'] = _T('newsletter:subscribe_message_ok', ['email' => "<b>$email</b>"]);
	}

	include_spip('inc/nospam');
	if (function_exists('nospam_confirm_action_html')) {
		$html_confirm = nospam_confirm_action_html('subscribe', "Subscribe $email " . json_encode($options), [$email, $options], 'newsletter/');
