...
function newsletters_recuperer_fond($id_newsletter, $patron, $date = null, $date_precedente = null){
	...
	// definir l'URL de base de la newsletter pour les urls absolues si besoin
	newsletter_set_url_base($id_newsletter);
	...
	$mem_cible_public = ($GLOBALS['lien_implicite_cible_public'] ?? null);
	...
	if (is_null($mem_cible_public)) {
		unset($GLOBALS['lien_implicite_cible_public']);
	}
	else {
		$GLOBALS['lien_implicite_cible_public'] = $mem_cible_public;
	}
	newsletter_set_url_base();
