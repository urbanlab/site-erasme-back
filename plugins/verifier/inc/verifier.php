...
function inc_verifier_dist($valeur, $type, $options = null, &$valeur_normalisee = null) {
	...
	$valeur_vide = (is_null($valeur) || (is_string($valeur) && $valeur == ''));
	// Si la valeur est vide, il n'y a rien a verifier donc c'est bon
	if (
		$valeur_vide
		&& empty($options['normaliser'])
	) {
		return '';
	}
	...
	if (!isset($verifications[$repertoire])) {
		...
		if (count($liste)) {
			foreach ($liste as $fichier => $chemin) {
				...
				if (
					charger_fonction($type, $repertoire, true)
					&& (
...
function verifier_trier_par_titre($array1, $array2) {
	if (isset($array1['titre']) && isset($array2['titre'])) {
...
function verifier_charger_infos($type_verif, $repertoire = 'verifier') {
	...
	if (test_plugin_actif('yaml')) {
		...
		if (is_array($verif)) {
			$verif['titre'] = ($verif['titre'] ?? '') ? appliquer_filtre($verif['titre'], '_T_ou_typo', true) : $type_verif;
			$verif['description'] = ($verif['description'] ?? '') ? appliquer_filtre($verif['description'], '_T_ou_typo', true) : '';
			$verif['icone'] = ($verif['icone'] ?? '') ? appliquer_filtre($verif['icone'], '_T_ou_typo', true) : '';
		}
	}
	return $verif;
}
